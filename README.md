...
## 프로젝트 소개
**KickSync**는 입점사 기반의 플랫폼인 KREAM, StockX를 모티브로 한 대용량 트래픽 처리 프로젝트입니다.

플랫폼 성장에 따라 급증하는 일일 100만 건 이상의 판매 데이터를 입점사별로 빠르고 정확하게 정산하는 시스템 구축에 주력했습니다. 대규모 트래픽 환경에서도 금전적 데이터의 정합성을 100% 보장하고, 인프라 확장에 유연한 아키텍처로 고도화했습니다.
...
**Step 4. I/O 병목 해결을 위한 수평적 확장 (Partitioning)**
- **[분석]** 단일 스레드로는 I/O 대기로 인해 DB 자원을 100% 활용하지 못함을 확인. DB 스케일업 대신, 애플리케이션 레벨의 파티셔닝을 통해 처리량을 극대화하는 전략을 선택.
- **[해결]** Spring Batch Partitioning 도입. 트래픽이 적은 새벽 시간대(04:00)의 가용 리소스를 독점적으로 활용하기 위해 **GridSize=10**으로 설정, 10개의 Worker가 병렬로 트랜잭션을 수행하여 처리 효율 극대화.
...
**해결 (안전한 락 제어)**
- **[설정]** 'waitTime=5s', 'leaseTime=10s' 적용
  - 락 획득 실패 시 빠르게 예외를 던져 UX 보호, 서버 장애 시 락이 영구 점유되는 데드락 상황 방지
- **[AOP 적용]** Custom AOP(@DistributedLock)를 구현하여 **'락 획득 → 트랜잭션 시작 → 커밋 → 락 해제'**의 생명주기를 강제, @Transactional과의 순서 문제 해결

**결과**
- nGrinder 동시성 테스트 결과 **재고 오차 0건 달성** (하단 로그 참조)
