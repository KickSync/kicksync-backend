...
* **문제 상황:** 전체 상품 목록 조회 API 호출이 빈번하여 DB CPU 부하가 증가하고 트래픽 급증 시 커넥션 풀 고갈 위험 감지.
...
* **검증 결과 및 판단 근거:**
   ...
   * **판단 근거:** '상품 목록'은 다수의 사용자에게 빠르게 노출되어야 하는 **전시 도메인**. 따라서 **미세한 데이터 반영 지연보다는 응답 속도 확보와 DB 보호가 비즈니스적으로 더 중요하다고 판단**. (전시 도메인 특성 고려)
...
* **향후 개선안:**
     * **Fallback 메커니즘 구축:** Redis 장애 시 DB 비관적 락으로 자동 전환되는 Fallback 프로세스 구현. 성능 Trade-off를 감수하더라도 주문 서비스의 가용성을 확보할 예정입니다.
